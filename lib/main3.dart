// ════════════════════════════════════════════════════════════════
// 🐱 MEOWSCAN v3.0 — main_v17.dart
// ════════════════════════════════════════════════════════════════
// Tema: Claro, colorido, redondeado y playful
// Fuente: DM Sans (moderna, limpia, juvenil)
// Colores: Coral, Turquesa, Amarillo, Lavanda
// FIX v17: History bug — _refresh() ahora actualiza UI desde
//   SharedPreferences primero (sin race condition con Firestore),
//   luego sincroniza Firestore en background.
// ════════════════════════════════════════════════════════════════
import 'dart:async';
import 'dart:convert';
import 'dart:io';
...
BoxDecoration kCardDeco({Color? color, Color? border, double radius = 22}) =>
  BoxDecoration(
    color: color ?? kCard,
...
class _MainShellState extends State<MainShell> {
  ...
  // FIX v17: Primero actualiza desde SharedPreferences (datos frescos, sin esperar Firestore).
  // Luego sincroniza Firestore en background. Esto evita la race condition donde
  // FirestoreService.loadEscaneos() devuelve lista vacía justo después de un batch.commit().
  void _refresh() async {
    // PASO 1: Actualizar UI inmediatamente desde SharedPreferences
    // (saveScans/saveCats se llaman ANTES de onComplete() en cada pantalla de escaneo,
    //  por lo que los datos locales ya son frescos)
    try {
      final localCats  = await StorageService.loadCats();
      final localScans = await StorageService.loadScans();
      final localAppts = await StorageService.loadAppointments();
      final localMeds  = await StorageService.loadMedications();
      _user.cats         = localCats;
      _user.scans        = localScans;
      _user.appointments = localAppts;
      _user.medications  = localMeds;
      if (mounted) setState(() {});
    } catch (e) {
      debugPrint('_refresh local error: \$e');
      if (mounted) setState(() {});
    }
    // PASO 2: Sincronizar con Firestore en background (sin bloquear la UI)
    Future.microtask(() async {
      try {
        ...
        // Solo sobrescribir si Firestore devuelve datos válidos
        // (evitar borrar datos locales si Firestore devuelve vacío por lag)
        if (scans.isNotEmpty || _user.scans.isEmpty) {
          _user.cats         = cats;
          _user.scans        = scans;
          _user.appointments = appts;
          _user.medications  = meds;
          await StorageService.saveCats(_user.cats);
          await StorageService.saveScans(_user.scans);
          await StorageService.saveAppointments(_user.appointments);
          await StorageService.saveMedications(_user.medications);
          if (mounted) setState(() {});
        }
      } catch (e) {
        debugPrint('_refresh firestore sync error: \$e');
      }
    });
  }
  ...
  Widget _navBar() {
    final items = [
      {'icon': Icons.home_rounded,               'label': L.lang=='en'?'Scan':'Escanear'},
      {'icon': Icons.history_rounded,            'label': L.lang=='en'?'History':'Historial'},
      {'icon': Icons.medical_services_rounded,   'label': 'VetBot'},
      {'icon': Icons.pets_rounded,               'label': L.lang=='en'?'My Pets':'Mascotas'},
      {'icon': Icons.settings_rounded,           'label': L.lang=='en'?'Settings':'Config'},
    ];
    return Container(
      decoration: BoxDecoration(
        color: kSurface,
                ...
                child: AnimatedContainer(
                  ...
                  child: Row(mainAxisSize: MainAxisSize.min, children: [
                    Icon(items[i]['icon'] as IconData,
                      color: active ? kPurple : kMuted,
                      size: active ? 22 : 20),
                    if (active) ...[
                      const SizedBox(width: 6),
                      Text(items[i]['label'] as String,
                        style: _nunito(12, kPurple, weight: FontWeight.w800)),
                    ],
                  ]),
                ),
              );
...
class _HomeTabState extends State<HomeTab> {
    ...
    return Container(
      ...
      decoration: const BoxDecoration(
        gradient: LinearGradient(
          ...
          colors: [kPurple, Color(0xFF9B59B6), kCoral],
        ),
      ...
      child: SafeArea(
        child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          ...
          Row(children: [
            ...
            Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
              Text('${L.get('welcome')}, ${widget.user.username} 👋',
                style: _nunito(13, Colors.white.withOpacity(0.85))),
              Text('MeowScanAI',
                style: _nunito(19, Colors.white, weight: FontWeight.w900)),
            ])),
            GestureDetector(
              onTap: () {},
              child: Container(
          ...
          const SizedBox(height: 16),
          Row(children: [
            _statusChip('🔬', '${widget.user.scans.length} ${L.lang=='en'?'scans':'escaneos'}'),
            const SizedBox(width: 8),
            _statusChip('💊', '${widget.user.medications.length} ${L.lang=='en'?'meds':'meds'}'),
            const SizedBox(width: 8),
            _statusChip('📅', '${widget.user.appointments.length} ${L.lang=='en'?'appts':'citas'}'),
          ]),
        ]),
      ),
    );
  ...
  Widget _statusChip(String emoji, String label) => Container(
    ...
    child: Row(mainAxisSize: MainAxisSize.min, children: [
      ...
      Text(label, style: _nunito(11, Colors.white, weight: FontWeight.w700)),
    ]),
  );
        ...
        Padding(
          ...
          child: Row(children: [
            Expanded(child: Column(
              ...
              children: [
                Text(L.get('scan_title') ?? (L.lang=='en'?'Scan your pet':'Escanear mascota'),
                  style: _nunito(20, Colors.white, weight: FontWeight.w900)),
                const SizedBox(height: 4),
                Text(L.get('scan_sub') ?? (L.lang=='en'?'AI analysis in seconds':'Análisis IA en segundos'),
                  style: _nunito(12, Colors.white.withOpacity(0.8))),
                const SizedBox(height: 14),
                Container(
                  padding: const EdgeInsets.symmetric(horizontal: 16, vertical: 8),
                  decoration: BoxDecoration(
                    color: Colors.white,
                    borderRadius: BorderRadius.circular(50)),
                  child: Text(L.lang=='en'?'Open camera →':'Abrir cámara →',
                    style: _nunito(12, kPurple, weight: FontWeight.w800))),
              ])),
      ...
      Row(mainAxisAlignment: MainAxisAlignment.spaceBetween, children: [
        ...
        GestureDetector(
          ...
          child: Container(
            ...
            decoration: BoxDecoration(
              gradient: const LinearGradient(colors: [kPurple, kCoral]),
              borderRadius: BorderRadius.circular(20)),
            child: Row(mainAxisSize: MainAxisSize.min, children: [
              const Icon(Icons.add_rounded, color: Colors.white, size: 14),
              const SizedBox(width: 4),
              Text(L.lang=='en'?'Add pet':'Agregar',
      ...
      if (cats.isEmpty)
        Container(
          ...
          child: Column(children: [
            ...
            const SizedBox(height: 8),
            Text(L.get('no_cats') ?? (L.lang=='en'?'No pets yet':'Sin mascotas aún'),
              style: _nunito(14, kMuted, weight: FontWeight.w700),
              ...
              return GestureDetector(
                ...
                child: AnimatedContainer(
                  ...
                  child: Column(mainAxisAlignment: MainAxisAlignment.center, children: [
                    ...
                    const SizedBox(height: 4),
                    Text(cat.name,
                      style: _nunito(10, sel ? Colors.white : kText,
...
class _HistoryTabState extends State<HistoryTab> {
  ...
  @override
  Widget build(BuildContext context) {
    // Show ALL scans sorted by date (newest first)
    final validCatIds = widget.user.cats.map((c) => c.id).toSet();
    final scans = List<ScanRecord>.from(widget.user.scans)
            ...
            Expanded(child: ListView.separated(
              ...
              itemBuilder: (_, i) {
                ...
                final cat = widget.user.cats.firstWhere(
                  ...
                  orElse: () => CatProfile(
                      id: s.catId,
                      name: s.catId == 'general'
                          ? (L.lang == 'en' ? 'General' : 'General')
                          : (L.lang == 'en' ? 'Deleted pet' : 'Mascota eliminada'),
    ...
    return Dismissible(
      ...
      child: GestureDetector(
        ...
        child: Container(
          ...
          child: Row(children: [
            Container(width: 4, height: double.infinity,
              decoration: BoxDecoration(
          ...
          ]),  // close outer Row (coloredBorder + Expanded)
        ),
      ),
    );
  }
