...
class VaccineRecord {
  ...
  static List<String> commonVaccines(String tipo) => tipo == 'perro'
    ? ['Parvovirus','Moquillo','Hepatitis','Parainfluenza',
       'Rabia','Bordetella','Leptospirosis','Coronavirus']
    : ['Triple Felina (HCP)','Rabia','Leucemia Felina (FeLV)',
       'Panleucopenia','Herpesvirus','Calicivirus','Bordetella Felina'];
}
...
class FirestoreService {
  ...
  static Future<void> saveVaccines(List<VaccineRecord> vaccines) async {
    ...
    for (final v in vaccines) {
      batch.set(col.doc(v.id), v.toJson());
    }
    await batch.commit();
  }
...
class NotificationService {
    ...
    try {
      ...
      if (sevenDays.isAfter(now)) {
        await _plugin.zonedSchedule(
          ...
          L.lang == 'en'
            ? '${vac.name} — schedule your vet visit'
            : '${vac.name} — agenda tu cita veterinaria',
      ...
      if (oneDay.isAfter(now)) {
        await _plugin.zonedSchedule(
          ...
          L.lang == 'en'
            ...
            : '💉 ¡Vacuna vence mañana!',
          L.lang == 'en'
            ? '${vac.name} — don\'t forget your vet appointment'
            : '${vac.name} — no olvides tu cita veterinaria',
      ...
      // El día que vence
      if (vac.nextDoseDate!.isAfter(now)) {
        await _plugin.zonedSchedule(
          _vacId(vac.id, 2),
          L.lang == 'en'
            ? '🚨 Vaccine expired today!'
            : '🚨 ¡Vacuna vencida hoy!',
          L.lang == 'en'
            ? '${vac.name} expired — see your vet as soon as possible'
            : '${vac.name} venció — ve al veterinario lo antes posible',
          tz.TZDateTime.from(vac.nextDoseDate!, tz.local),
          _details('meowscan_appts', 'Vet Appointments'),
          androidScheduleMode: AndroidScheduleMode.alarmClock,
          uiLocalNotificationDateInterpretation:
            UILocalNotificationDateInterpretation.absoluteTime,
  ...
  static Future<void> cancelVaccine(String vacId) async {
    try {
      ...
      await _plugin.cancel(_vacId(vacId, 2));
    } catch (_) {}
  }
...
class _VaccineScreenState extends State<VaccineScreen> {
        ...
        builder: (ctx, setS) => Container(
          ...
          child: SingleChildScrollView(child: Column(
            ...
            children: [
              ...
              Wrap(spacing: 8, runSpacing: 8,
                children: common.map((v) => GestureDetector(
                  ...
                  child: AnimatedContainer(
                    ...
                    padding: const EdgeInsets.symmetric(horizontal: 12, vertical: 6),
                    decoration: BoxDecoration(
                      color: selectedCommon == v
                        ? kTurquoise.withOpacity(0.15)
                        : kBg,
                      borderRadius: BorderRadius.circular(20),
                      border: Border.all(
              ...
              kLabel(L.lang == 'en' ? 'VETERINARIAN' : 'VETERINARIO'),
              const SizedBox(height: 8),
              kTextField(vetCtrl,
                L.lang == 'en' ? 'Vet name (optional)' : 'Nombre del veterinario (opcional)',
                icon: Icons.person_rounded, accent: kTurquoise),
              const SizedBox(height: 14),

              kLabel(L.lang == 'en' ? 'LOT NUMBER' : 'NÚMERO DE LOTE'),
              const SizedBox(height: 8),
              kTextField(lotCtrl,
                L.lang == 'en' ? 'Lot number (optional)' : 'Número de lote (opcional)',
                icon: Icons.tag_rounded, accent: kTurquoise),
              const SizedBox(height: 14),
                ...
                child: Container(
                  ...
                  child: Row(children: [
                    const Icon(Icons.calendar_today_rounded,
                        color: kTurquoise, size: 18),
                    const SizedBox(width: 10),
                    Text(DateFormat('dd/MM/yyyy').format(appDate),
                      style: _nunito(14, kText)),
                  ]))),
              const SizedBox(height: 14),

              // Próxima dosis
              kLabel(L.lang == 'en' ? 'NEXT DOSE DATE (OPTIONAL)' : 'FECHA PRÓXIMA DOSIS (OPCIONAL)'),
              const SizedBox(height: 8),
              GestureDetector(
                onTap: () async {
                  final d = await showDatePicker(
                  ...
                  child: Row(children: [
                    ...
                    Text(
                      nextDate != null
                        ...
                        : (L.lang == 'en' ? 'Tap to set date' : 'Toca para establecer fecha'),
                      style: _nunito(14,
                        nextDate != null ? kText : kMuted)),
                    if (nextDate != null) ...[
                      const Spacer(),
                      GestureDetector(
              ...
              kLabel(L.lang == 'en' ? 'NOTES' : 'NOTAS'),
              const SizedBox(height: 8),
              kTextField(notesCtrl,
                L.lang == 'en' ? 'Reactions, observations...' : 'Reacciones, observaciones...',
                icon: Icons.notes_rounded, accent: kTurquoise),
              const SizedBox(height: 14),
              ...
              Container(
                ...
                child: Row(children: [
                  ...
                  Expanded(child: Text(
                    L.lang == 'en'
                      ? 'Remind me before next dose'
                      : 'Recordarme antes de la próxima dosis',
                    style: _nunito(13, kText))),
                  Switch(
                    value: notify,
  ...
  void _deleteVaccine(VaccineRecord vac) async {
    final ok = await showDialog<bool>(
      ...
      builder: (_) => AlertDialog(
        ...
        content: Text(
          L.lang == 'en'
            ? 'This vaccine record will be deleted.'
            : 'Se eliminará este registro de vacuna.',
          style: _nunito(13, kMuted)),
        actions: [
          TextButton(
            onPressed: () => Navigator.pop(context, false),
        ...
        child: Column(children: [
            ...
            child: Row(children: [
              ...
              AnimatedPressButton(
                ...
                child: Container(
                  ...
                  decoration: BoxDecoration(
                    gradient: const LinearGradient(
                      colors: [kTurquoise, Color(0xFF00CEC9)]),
                    borderRadius: BorderRadius.circular(14)),
                  child: Row(mainAxisSize: MainAxisSize.min, children: [
          ...
          if (vaccines.isNotEmpty)
            Padding(
              ...
              child: Row(children: [
                _statChip('✅', '$ok ${L.lang == 'en' ? 'ok' : 'al día'}',
                  const Color(0xFF00B894)),
                const SizedBox(width: 8),
                _statChip('⚠️', '$soon ${L.lang == 'en' ? 'soon' : 'pronto'}',
                  kYellow),
                const SizedBox(width: 8),
                _statChip('🚨', '$expired ${L.lang == 'en' ? 'expired' : 'vencidas'}',
                  kCoral),
              ])),
            ...
            child: vaccines.isEmpty
              ? Center(child: Column(
                  ...
                  children: [
                    ...
                    kBody(
                      L.lang == 'en'
                        ? 'Tap + Add to register the first one'
  ...
  Widget _vaccineCard(VaccineRecord vac) {
    ...
    final daysLeft = vac.nextDoseDate != null
      ? vac.nextDoseDate!.difference(DateTime.now()).inDays
      : null;
    ...
    return Container(
      ...
      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        ...
        Row(children: [
          ...
          if (vac.nextDoseDate != null)
            Expanded(child: _dateInfo(
              ...
              DateFormat('dd/MM/yyyy').format(vac.nextDoseDate!),
              color)),
        ]),
            ...
            child: Text(
              daysLeft < 0
                ? (L.lang == 'en'
                    ...
                    : '🚨 Vencida hace ${daysLeft.abs()} días')
                : daysLeft == 0
        ...
        // Notas y lote
        if (vac.notes.isNotEmpty || vac.lotNumber.isNotEmpty) ...[
          const SizedBox(height: 8),
          if (vac.lotNumber.isNotEmpty)
            kBody('Lote: ${vac.lotNumber}', color: kMuted, size: 11),
