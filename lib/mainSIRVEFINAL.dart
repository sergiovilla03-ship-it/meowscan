...
class _PetTranslatorScreenState extends State<PetTranslatorScreen>
  ...
  // ── Quemar subtítulos en el celular con ffmpeg_kit ──
  Future<String?> _quemarSubtitulosLocal(
      String videoPath, String frase, int prob) async {
    try {
      final dir      = await getTemporaryDirectory();
      final outPath  = '${dir.path}/meowscan_sub_${DateTime.now().millisecondsSinceEpoch}.mp4';
      final nombre   = widget.cat.name;
      final probLbl  = L.lang == 'en' ? 'probability' : 'probabilidad';
      final marca    = 'MeowScanAI  |  $prob% $probLbl';

      // Escapar comillas simples para ffmpeg
      final fraseEsc = frase.replaceAll("'", "\\'").replaceAll(':', '\\:');
      final marcaEsc = marca.replaceAll("'", "\\'").replaceAll(':', '\\:');

      final cmd = '-i $videoPath '
          '-vf "drawtext=text=\'$fraseEsc\':fontsize=26:fontcolor=white:'
          'x=(w-text_w)/2:y=h-80:box=1:boxcolor=black@0.65:boxborderw=8,'
          'drawtext=text=\'$marcaEsc\':fontsize=13:fontcolor=white@0.8:'
          'x=10:y=10:box=1:boxcolor=black@0.4:boxborderw=4" '
          '-c:v libx264 -c:a aac -crf 23 -preset ultrafast '
          '-y $outPath';
      ...
      if (ReturnCode.isSuccess(returnCode)) {
        print('✅ Subtítulos quemados localmente: $outPath');
        return outPath;
      } else {
        final log = await session.getFailStackTrace();
        print('⚠️ ffmpeg_kit falló: $log');
        return null;
      }
    } catch (e) {
      print('⚠️ _quemarSubtitulosLocal error: $e');
      return null;
    }
  }
    ...
    try {
      ...
      if (res.statusCode == 200) {
        ...
        if (ct.contains('video')) {
          ...
          // Quemar subtítulos localmente con ffmpeg_kit
