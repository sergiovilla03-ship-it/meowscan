...
class _PetTranslatorScreenState extends State<PetTranslatorScreen>
  ...
  Future<void> _startRecording() async {
    ...
    if (!ok.isGranted) return;
    final dir  = await getTemporaryDirectory();
    final path = '${dir.path}/translator_${DateTime.now().millisecondsSinceEpoch}.m4a';
    await _recorder.start(const RecordConfig(encoder: AudioEncoder.aacLc), path: path);
    setState(() { _recording = true; _secs = 0; _dbHistory = []; _resultado = null; });
    _timer = Timer.periodic(const Duration(seconds: 1), (t) async {
      final amp = await _recorder.getAmplitude();
      final db  = amp.current.isFinite ? amp.current : -60.0;
      setState(() {
  ...
  Future<void> _stopAndTranslate() async {
    ...
    final loudSecs = _dbHistory.where((d) => d > -30).length;
    final silSecs  = _dbHistory.where((d) => d < -50).length;
    final tipo     = widget.cat.tipo == 'perro' ? 'dog' : 'cat';

    final desc = loudSecs == 0 && silSecs > _maxSecs * 0.8
      ? "The pet was mostly silent during the recording."
      : "Recording of $_secs seconds. "
        "Average volume: ${avgDb.toStringAsFixed(1)} dB. "
        "Active sound seconds: $loudSecs of $_secs. "
        "Silent seconds: $silSecs of $_secs. "
        "${loudSecs > 5 ? 'The pet made frequent and intense sounds.' : ''}"
        "${loudSecs >= 2 && loudSecs <= 5 ? 'The pet made occasional sounds.' : ''}"
        "${silSecs > _maxSecs * 0.7 ? 'The pet remained mostly quiet.' : ''}";

    try {
      final ip    = widget.serverIp;
      final proto = ip.contains("onrender") || ip.contains("trycloudflare") ? "https" : "http";
      final port  = ip.contains("onrender") || ip.contains("trycloudflare") ? "" : ":8000";
      final uri   = Uri.parse("$proto://$ip${port}/traducir_mascota");
      final res   = await http.post(uri,
        headers: {"Content-Type": "application/json", "X-API-Key": kApiKey},
        body: json.encode({
          "lang":        L.lang,
          "tipo":        tipo,
          "nombre":      widget.cat.name,
          "descripcion": desc,
          "duracion_seg":_secs.toDouble(),
          "intensidad_db": avgDb,
        })).timeout(const Duration(seconds: 20));
      ...
      if (res.statusCode == 200 && mounted) {
        setState(() { _resultado = json.decode(res.body); _analyzing = false; });
      } else {
        // Fallback gracioso si el servidor no tiene el endpoint aún
        setState(() {
          _resultado = _fallbackResult();
          _analyzing = false;
        });
      }
    } catch (_) {
      if (mounted) setState(() { _resultado = _fallbackResult(); _analyzing = false; });
    }
  }

  // Fallback mientras el backend no tenga el endpoint
  Map<String, dynamic> _fallbackResult() {
    ...
    final frases = esDog ? [
      ...
      {"frase": L.lang == 'en' ? "You're my favorite human, I love you." : "Eres mi humano favorito, te quiero.", "prob": 78, "emoji": "🐶"},
      {"frase": L.lang == 'en' ? "The mailman is SUSPICIOUS. I'm watching him." : "El cartero es SOSPECHOSO. Lo estoy vigilando.", "prob": 95, "emoji": "🐕‍🦺"},
    ] : [
      {"frase": L.lang == 'en' ? "Feed me. Feed me NOW. I said feed me." : "Dame comida. AHORA. Te dije que me dieras comida.", "prob": 94, "emoji": "😾"},
      ...
      {"frase": L.lang == 'en' ? "This house belongs to me. You just pay the rent." : "Esta casa es mía. Tú solo pagas el arriendo.", "prob": 91, "emoji": "🐱"},
    ];
    frases.shuffle();
    ...
    return {
      ...
      "emoji":       chosen["emoji"],
      "contexto":    L.lang == 'en'
        ? "Based on the sound pattern and $nombre's personality"
        : "Basado en el patrón de sonido y la personalidad de $nombre",
  ...
  void _shareResult() async {
    ...
    final nombre = widget.cat.name;
    final text = L.lang == 'en'
      ? '🐾 $nombre says (${prob}% probability):
"$frase"

Translated with MeowScanAI 🤖
#PetTranslator #MeowScan #AIpets'
      : '🐾 $nombre dice (${prob}% de probabilidad):
"$frase"

Traducido con MeowScanAI 🤖
#TraductorDeMascotas #MeowScan #IA';
    await Share.share(
      text,
  ...
  Widget _recordView(bool isDog) {
    final dbNorm = ((_currentDb + 60) / 60).clamp(0.0, 1.0);

    return Column(mainAxisAlignment: MainAxisAlignment.center, children: [
      // Descripción
      Padding(
        padding: const EdgeInsets.symmetric(horizontal: 32),
        child: Text(L.get('translator_sub'),
          style: _nunito(16, Colors.white60),
          textAlign: TextAlign.center)),
      const SizedBox(height: 48),

      // Círculo animado de grabación
      AnimatedBuilder(
        ...
        builder: (_, __) => Stack(alignment: Alignment.center, children: [
          ...
          if (_recording)
            Container(
              width: 180 + (_pulseCtrl.value * 30),
              height: 180 + (_pulseCtrl.value * 30),
              decoration: BoxDecoration(
                shape: BoxShape.circle,
                color: kPurple.withOpacity(0.08 * _pulseCtrl.value))),
          // Onda media (dB reactiva)
          if (_recording)
            Container(
              width: 160 + (dbNorm * 20),
              height: 160 + (dbNorm * 20),
              decoration: BoxDecoration(
                shape: BoxShape.circle,
          ...
          AnimatedPressButton(
            ...
            child: Container(
              ...
              decoration: BoxDecoration(
                ...
                boxShadow: [BoxShadow(
                  color: (_recording ? kCoral : kPurple).withOpacity(0.4),
              ...
              child: Center(child: Column(
                ...
                children: [
                  ...
                  const SizedBox(height: 6),
                  Text(
                    _recording ? "$_secs/$_maxSecs seg" : L.get('translator_btn'),
                    style: _nunito(11, Colors.white70),
                    textAlign: TextAlign.center),
                ]))),
          ),
      ...
      Text(
        _analyzing
          ? L.get('translator_analyze')
          : _recording
            ? L.get('translator_record')
            : (L.lang == 'en'
                ? "Tap the button and let ${widget.cat.name} speak"
                : "Toca el botón y deja hablar a ${widget.cat.name}"),
        style: _nunito(14, Colors.white60),
        textAlign: TextAlign.center),
      ...
      Padding(
        ...
        child: Container(
          ...
          child: Text(
            L.lang == 'en'
              ? isDog
                ? "💡 Tip: Get ${widget.cat.name} to bark, whine or growl for better results"
                : "💡 Tip: Get ${widget.cat.name} to meow, purr or chirp for best results"
              : isDog
                ? "💡 Consejo: Haz que ${widget.cat.name} ladre, lloriquee o gruña para mejores resultados"
                : "💡 Consejo: Haz que ${widget.cat.name} maúlle, ronronee o chille para mejores resultados",
            style: _nunito(12, Colors.white38),
            textAlign: TextAlign.center))),
    ]);
  }
      ...
      child: Column(children: [
        ...
        Container(
          ...
          child: Column(children: [
            ...
            // Marca de agua
            Text('MeowScanAI 🐾',
              style: _nunito(10, Colors.white24)),
          ])),

        const SizedBox(height: 20),

        // Botón compartir (viral)
        AnimatedPressButton(
          onTap: _shareResult,
          child: Container(
            width: double.infinity,
            ...
            child: Row(mainAxisAlignment: MainAxisAlignment.center, children: [
              const Text("📲", style: TextStyle(fontSize: 20)),
              const SizedBox(width: 10),
              Text(L.get('translator_share'),
                style: _nunito(15, Colors.white, weight: FontWeight.w800)),
            ]))),

        const SizedBox(height: 12),

        // Botón traducir de nuevo
        kOutlineBtn(
          L.get('translator_again'),
          () => setState(() { _resultado = null; _secs = 0; }),
          color: kPurple),
