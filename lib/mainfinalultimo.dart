...
class _PetTranslatorScreenState extends State<PetTranslatorScreen>
  ...
  Future<void> _stopAndTranslate() async {
    ...
    try {
      ...
      if (res.statusCode == 200) {
        // El backend puede devolver JSON (solo texto) o video procesado
        final ct = res.headers['content-type'] ?? '';
        if (ct.contains('video')) {
          // Guardar video procesado con subtítulos
          final dir  = await getTemporaryDirectory();
          ...
          // Solo JSON con la traducción
          setState(() { _resultado = json.decode(res.body); });
        }
      } else {
        setState(() { _resultado = _fallbackResult(); });
      }
    } catch (_) {
      setState(() {
        _uploading  = false;
